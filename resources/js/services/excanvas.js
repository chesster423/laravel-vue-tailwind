...
if (!document.createElement('canvas').getContext) {
  (function () {
    ...
    const IE_VERSION = +navigator.userAgent.match(/MSIE ([\d.]+)?/)[1];
    ...
    function getRgbHslContent(styleString) {
      ...
      const parts = styleString.substring(start + 1, end).split(',');
      // add alpha if needed
    ...
    function getComputedStyle(style, element) {
      ...
      // Compute the size
      const canvasFontSize = parseFloat(element.currentStyle.fontSize),
    ...
    function CanvasRenderingContext2D_(canvasElement) {
      ...
      const el = canvasElement.ownerDocument.createElement('div');
      el.style.cssText = cssText;
      canvasElement.appendChild(el);

      const overlayEl = el.cloneNode(false);
    ...
    contextPrototype.bezierCurveTo = function (
      ...
      const p = getCoords(this, aX, aY);
      const cp1 = getCoords(this, aCP1x, aCP1y);
      const cp2 = getCoords(this, aCP2x, aCP2y);
    ...
    contextPrototype.quadraticCurveTo = function (aCPx, aCPy, aX, aY) {
      ...
      const cp = getCoords(this, aCPx, aCPy);
      const p = getCoords(this, aX, aY);
    ...
    contextPrototype.arc = function (
      ...
      const p = getCoords(this, aX, aY);
      const pStart = getCoords(this, xStart, yStart);
      const pEnd = getCoords(this, xEnd, yEnd);
    ...
    contextPrototype.drawImage = function (image, var_args) {
      ...
      if (
        ...
        const max = d;
        const c2 = getCoords(this, dx + dw, dy);
        const c3 = getCoords(this, dx, dy + dh);
        const c4 = getCoords(this, dx + dw, dy + dh);
    ...
    function appendStroke(ctx, lineStr) {
      const a = processStyle(ctx.strokeStyle);
      const color = a.color;
      let opacity = a.alpha * ctx.globalAlpha;
      const lineWidth = ctx.lineScale_ * ctx.lineWidth;
    ...
    function appendFill(ctx, lineStr, min, max) {
      ...
      const arcScaleX = ctx.arcScaleX_;
      const arcScaleY = ctx.arcScaleY_;
      ...
      if (fillStyle instanceof CanvasGradient_) {
        ...
        if (fillStyle.type_ == 'gradient') {
          const x0 = fillStyle.x0_ / arcScaleX;
          const y0 = fillStyle.y0_ / arcScaleY;
          const x1 = fillStyle.x1_ / arcScaleX;
          const y1 = fillStyle.y1_ / arcScaleY;
          var p0 = getCoords(ctx, x0, y0);
          const p1 = getCoords(ctx, x1, y1);
          const dx = p1.x - p0.x;
          const dy = p1.y - p0.y;
        ...
        const opacity1 = stops[0].alpha * ctx.globalAlpha;
        const opacity2 = stops[length - 1].alpha * ctx.globalAlpha;
        ...
        if (width && height) {
          const deltaLeft = -min.x;
          const deltaTop = -min.y;
          lineStr.push(
            '<g_vml_:fill',
            ' position="',
        ...
        const a = processStyle(ctx.fillStyle);
        const color = a.color;
        const opacity = a.alpha * ctx.globalAlpha;
        lineStr.push(
          '<g_vml_:fill color="',
          color,
          '" opacity="',
    ...
    contextPrototype.drawText_ = function (text, x, y, maxWidth, stroke) {
      ...
      const fontStyle = getComputedStyle(
        processFontStyle(this.font),
        this.element_,
      );

      const fontStyleString = buildStyle(fontStyle);

      const elementStyle = this.element_.currentStyle;
      ...
      const d = getCoords(this, x + offset.x, y + offset.y);
      ...
      const skewOffset = mr(d.x / Z) + ',' + mr(d.y / Z);
    ...
    const p = (DOMException_.prototype = new Error());
    p.INDEX_SIZE_ERR = 1;
